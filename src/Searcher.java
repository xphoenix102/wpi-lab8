...
public class Searcher
{
    ...
    public static void main(String args[])
    {
        ...
        TermQuery tq1;
        {
            ...
            Term t = new Term(Constants.content,queryMammal.toLowerCase());
            tq1 = new TermQuery(t);
...
//            printResultsForQuery(indexSearcher,tq2);
        ...
        // Boolean query
        {
            ...
            BooleanClause b1 = new BooleanClause(tq1,BooleanClause.Occur.SHOULD);
            BooleanClause b2 = new BooleanClause(tq2,BooleanClause.Occur.SHOULD);
            BooleanQuery.Builder bb = new BooleanQuery.Builder();

            bb.add(b1);
            bb.add(b2);
            bb.setMinimumNumberShouldMatch(2);
        ...
        // For this reason, construct PrefixQuery.
        {
            ...
            Term t = new Term(Constants.filename,"ant");
            //printResultsForQuery(indexSearcher,new PrefixQuery(t));
        ...
        // which contain a term "eat?" "?" stand for any letter (* for a sequence of letters).
        {
            ...
            Term t = new Term(Constants.content,"eat?");
            //printResultsForQuery(indexSearcher,new WildcardQuery(t));
        ...
        // Which documents have been found?
        {
            ...
            //printResultsForQuery(indexSearcher,new FuzzyQuery(new Term(Constants.content,"mamml?")));
            // --------------------------------------
        }
    ...
    private static void printResultsForQuery(IndexSearcher indexSearcher, Query q)
    {
        ...
        // --------------------------------
        try {
            TopDocs top = indexSearcher.search(q,Constants.top_docs);
            for(ScoreDoc sD : top.scoreDocs){
                System.out.println("Nazwa"+indexSearcher.doc(sD.doc).get(Constants.filename));
                System.out.println("Score"+sD.score);
                System.out.println("Id"+indexSearcher.doc(sD.doc).get(Constants.id));
                System.out.println("Nazwa"+indexSearcher.doc(sD.doc).get(Constants.filesize));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
